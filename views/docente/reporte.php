            ...
            <div class="navegation__bar col-lg-12 col-sm-12 p-4 border border-1 bg-primary-dark">
               ...
               <div class="row  p-3" id="listado_estudiantes">
                  <div class="col-lg-12 text-center text-white">
                     ...
                     <p>Reporte de Estudiantes del Curso</p>
                  </div>
...
for ($i = 0; $i < count($actividadesDisponibles); $i++) {
    echo '<th scope="col" width="10%">Tarea ' . $i + 1 . '
...
?>
                                 <th scope="col" width="10%">Total del Curso</th>
                              </tr>
                           </thead>
            ...
            if ($arrayEstudianteAsignatura[$i][3] == $arrayEstudiantes[$j][0]) {
                for ($h = 0; $h < count($arrayPersonas); $h++) {
                    if ($arrayPersonas[$h][0] == $arrayEstudiantes[$j][1]) {
                        echo '<tr ><th scope="col" width="5%"><input class="form-check-input" type="checkbox"></th>
                           <td class="flex justify-content-start">
                               ...
                               ' . $arrayPersonas[$h][1] . ' ' . $arrayPersonas[$h][2] .
                            '</td>';
                        for ($k = 0; $k < count($actividadesDisponibles); $k++) {
                            echo ' <td class="fw-bold" >' . floatval(rand(0.0, 9)) . '.' . floatval(rand(0.0, 9)) . '' . floatval(rand(0.0, 9)) . '</td>';
                        }

                        echo ' <td class="fw-bold bg-primary text-white" >' . floatval(rand(5, 9)) . '.' . floatval(rand(0.0, 9)) . '' . floatval(rand(0.0, 9)) . '</td>
                       </tr>';
                    }
                }
            }
